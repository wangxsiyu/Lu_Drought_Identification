...
for_allflumes <- function(d, func_perc, ...){
...
# Combined method
comb <- function(sf, doy, param_fr = list(), param_tlm = list(), param_cdpm = list()){
  ...
  perc = combineTC(sf, perc, fr, doy, param_fr$fr_option)
  

  comb = list(perc = perc)
  return(comb)
}
combineTC <- function(sf, perc, fr, doy, opt_fr){
  percnew = perc
  id = sf == 0
...
tlm <- function(sf, doy, param = list()){
  ...
  id = which(sf > 0)
  perc = matrix(NA,length(sf),1)
  perc[id] = calc_percentile(sf[id], doy[id], param$tlm_rgday)
