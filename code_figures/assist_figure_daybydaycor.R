...
plt_nacor <- function(data,flname){
  dailycor = calc_nacor(data,flname,option = 0)
  dailycor1 = calc_nacor(data,flname,option = 1)
    ...
    for (j in 1:length(percname)){
      ...
      for( dayi in 1: day365){
        ...
        if( length(idx_d) == 0 ){
          ...
          daycor[dayi] = cor(tperc[idx_d], daily_veg[idx_d], use = "pairwise", method = "spearman")
        }
      }
      daily_cor[[i,j]] = daycor
    }
